...
- **Python** — Declare compatible release ranges with PEP 440’s `~=` operator in `pyproject.toml`, lock with `uv lock`, and keep `.venv/` aligned via `uv sync --frozen`. Schedule (or approve) upgrades to track the newest safe releases and commit `uv.lock`.
- **Node** — Continue using pnpm with a committed `pnpm-lock.yaml`. In CI run `pnpm install --frozen-lockfile` and prime the store with `pnpm fetch` for deterministic, cache-friendly installs.
- **CI acceleration** — Cache package-manager stores with GitHub Actions cache entries keyed by the lockfiles. For offline/air-gapped paths, publish a wheelhouse (Python) or vendored store snapshot that cold-start runners can consume without hitting the public registries.
...
1. **Why it works** — PEP 440 ranges capture intent, `uv.lock` captures reality, and `uv sync --frozen` reproduces it precisely on every runner.
...
- **Go** — rely on modules, commit `go.sum`, cache the module + build caches, and run `go mod vendor` (or serve a module proxy) for offline work.
- **Rust** — commit `Cargo.lock`, cache the Cargo registry, and generate a `vendor/` directory with `cargo vendor` when air-gapped operation is required.
- **Java** — with Gradle, enable dependency locking (`./gradlew dependencies --write-locks`) and cache `~/.gradle`; with Maven, pre-fetch via `mvn dependency:go-offline` then build with `-o` and cache `~/.m2`.
...
- Use the repo-scoped `.npmrc` (sets `store-dir=./.pnpm-store`) so pnpm’s content-addressable store remains under version control boundaries. Run `pnpm cache:prune` to prune pnpm, uv, and pre-commit caches in one pass when disk space gets tight.
...
- Use `pnpm run sync` (backed by `scripts/sync-environments.sh`) to align uv-managed Python dependencies and the pnpm workspace quickly when lockfiles change but you do not need the full formatter and lint sweep.
...
- `.pnpm-store/` holds pnpm’s shared packages for this project; `pnpm cache:prune` wraps `pnpm store prune` so you do not have to remember the raw command.
- `.cache/pre-commit/` retains hook environments; the same `pnpm cache:prune` helper runs `pre-commit clean` under the hood, but you can also invoke `PRE_COMMIT_HOME=$PWD/.cache/pre-commit pre-commit clean` directly.
- `.cache/uv/` mirrors the uv wheel cache inside the repository (exported automatically by `scripts/setup-tooling.sh`); run `UV_CACHE_DIR=$PWD/.cache/uv uv cache prune --all` if you need to purge it separately.
