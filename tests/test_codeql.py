...
def test_codeql_manager_create_builds_database(
    ...
    monkeypatch.setattr(manager, '_fingerprint_source', lambda root, lang: 'abc123def456')

    source_root = tmp_path / 'src'
    source_root.mkdir()
    database = asyncio.run(manager.create_database(source_root=source_root, language='python'))
                    ...
                    'results': [
                        {
                            ...
                            'locations': [
                                {
                                    'physicalLocation': {
                                        'artifactLocation': {'uri': str(tmp_path / 'module.py')},
...
def test_codeql_manager_run_queries_handles_empty_queries(
    ...
    manager = CodeQLManager(codeql_path=tmp_path / 'codeql', cache_dir=tmp_path / 'cache')
    database = CodeQLDatabase(
        language='python',
        path=tmp_path,
        source_root=tmp_path,
...
def test_codeql_manager_discover_codeql_missing(monkeypatch: pytest.MonkeyPatch) -> None:
